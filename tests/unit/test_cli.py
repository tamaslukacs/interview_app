...
from fixtures import overwrite_file_parser_flow, runner  # noqa: F401; pylint: disable=unused-variable


@pytest.mark.parametrize("args, expected",
                         [(
                                 ["--DictionaryFile", get_example_input_filepath(),
                                  "--StartWord", "AAAA",
                                  "--EndWord", "BBBB",
                                  "--ResultFile", "res.txt"],
                                 [get_example_input_filepath(), "AAAA", "BBBB", "res.txt"]),
                             (["-in", get_example_input_filepath(),
                               "-start", 1111,
                               "-end", "GGGG",
                               "-out", "out.cfg"],
...
@pytest.mark.parametrize("args",
                         [(["-in", get_example_input_filepath(), "-start", "CCC", "-end", "SSS", "-out", "out.cfg"]),
                          (
                                  ["-in", get_example_input_filepath(), "-start", "ddddd", "-end", "fffff", "-out",
                                   "out.cfg"]),
...
@pytest.mark.parametrize("args",
                         [(["-in", "not_real_file.txt", "-start", 1111, "-end", "SSSS", "-out", "out.cfg"]),
                          (["-in", get_example_input_filepath(), "-start", "dddd", "-end", "ffff", "-out",
                            r"out ./\&=-cfg"]),
                          # (["-in", get_example_input_filepath(), "-start", "", "-end", "ffff", "-out", "out.cfg"])
                          ])
